...
public class Exam01_5 {
    public static void main(String[] args) {
        ...
        //byte + byte = int
        //연산을 하기 전에 byte 값이 int로 암시적 형변환이 된다.
        //byte r1 = b + b; //컴파일 오류

        //short + short = int;
        //=>연산을 하기 전에 short int로 암시적 형변환이 된다.
        //short r2 = s + s; //컴파일 오류
        ...
        //long l10 = b + s + i + l + f + d; //컴파일 오류
        
        //float + int + long = float
        //long r11 = f + i + l; //컴파일 오류
        
        //boolean + int = ?
        //=>산술연산자는 정수 타입(byte, short, char, int, long)과
        //  부동소수점 타입(float, double)에 대해서만
        //실행할 수 있다.
        //int r2 = bool + i; //컴파일 오류 //operand 피연산자
    }
}

//정리!
//연산은 항상 같은 타입끼리만 가능하다.
//그래서 다른 타입끼리 연산을 할 때는 둘 중 한 개의 타입을 다른 타입으로 바꿔야 한다.
//타입을 바꾸는 것을 내부적인 규칙에 따라 자동으로 처리한다고 해서
//'암시적 형변환(type casting, implicit type conversion'이라 부른다.
//암시적 형변환 규칙
