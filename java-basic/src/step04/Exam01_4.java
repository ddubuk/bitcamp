...
public class Exam01_4 {
    public static void main(String[] args) {
        int i = 5;
        int j = 2;
        float r = i / j; //int 와 int의 연산 결과는 항상 int이다.
                         //따라서 r 변수에 넣기 전에 이미 결과는 정수 2가 된다.
                         //정수 2를 float 변수에 넣으면 출력할 때 2.0이 된다.
        System.out.println(r);

        //해결책!
        //변수에 들어있는 값을 다른 타입으로 바꿔라
        //"형변환(type conversion = type castiong)"하라!
        ...
        int x = Integer.MAX_VALUE; //양수의 최대값 //0xfffffff = 약 21억
        int y = Integer.MAX_VALUE;              //0xfffffff =약 21억
        int r1 = x + y;                         //0xfffffffe = -2
        long r2 = x + y;                        //0xfffffffe = -2
        ...
        float f1 = 987.6543f;
        float f2 = 1.111111f;
        float r3 = f1 + f2; //988.765411 => 11부분은 잘린다.
        double r4 = f1 + f2;
        System.out.println(r3); //988.7654
        //출력 이유?
        //float과 float의 연산 결과는 float이다.
        //그래서 메모리 크기를 넘어가는 뒤의 11은 잘린다.
        //잘린 값이 r3와 r4에 저장된다.

        System.out.println(r4); //988.765380859375
        //출력 이유?
        //float 값을 double 변수에 저장할 때 정확하게 저장되지 않는다.
        //IEEE 754의 이진수 변환 문제 때문이다.
...
//float과 float의 연산결과는 float이다.
//즉, 연산 결과는 피연산자의 타입과 같다.
//그래서 두 값을 계산했을 때 메모리를 초과할 경우
//값이 잘릴 수 있다.
//주의!
//코드를 작성할 때 피연산자의 계산 결과가 피연산자의 메모리 크기를
//벗어날 가능성이 있다면, 처음부터 피연산자의 값을 더 큰 메모리에 담아서 메모리에 수행하라!
