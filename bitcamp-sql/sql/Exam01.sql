...
> insert into test1(c3) values(3.14659); /* 2자리를 초과한 값은 반올림. */
...
- 크기가 가변적이라서 검색할 때 데이터 위치를 계산해야 하기 때문에 검색 시 느리다.
 ...
 * 즉 c1='abc'에서는 데이터를 찾지 못하고 c1='abc  '여야만 데이털ㄹ 찾는 경우가 있다.
 * 그러나 mysql은 고정크기 칼럼이라도 빈자리를 무시하고 데이터를 찾는다.*/
...
- 두 개 이상의 컬럼에 대해서 개별적으로 PK를 지정해서는 안된다. 
...
- 그래서 실무에서는 이런 경우 '학번'처럼 임의의 컬럼을 만들어 PK로 사용한다.
> create table test1(
  no int primary key,
  name varchar(20),
  age int,
...
> insert into test1(no,name,age,kor,eng,math) values(4,'d',20,81,81,81);

/* 번호가 중복되었기 때문에 입력 거절 */
> insert into test1(no,name,age,kor,eng,math) values(4,'c',21,81,81,81);
...
검색할 때 사용한다.
```
select * from test11 where name = 'bbb';
```
