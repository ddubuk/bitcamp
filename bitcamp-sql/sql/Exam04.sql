...
create table test2(
  ...
  filepath varchar(255) not null, /* 파일 시스템에 저장된 첨부파일 경로 */
...
insert into test2(filepath, bno) values('c:/download/b.gif', 2);
insert into test2(filepath, bno) values('c:/download/c.gif', 3);
insert into test2(filepath, bno) values('c:/download/d.gif', 4);
...
insert into test2(filepath, bno) values('c:/download/g.gif', 100);
```
- 첨부 파일이 있는 게시물을 지울 수 있다.
...
alter table 테이블명
    add constraint 제약조건라벨이름 foreign key (컬럼명) references 테이블명(컬럼명);
예)
alter table test2
    add constraint test2_bno_fk foreign key(bno) references test1(no);
```
위와 같이 test2 테이블에 FK 제약 조건을
```
...
/* 그러나 5번 게시물을 삭제할 수 없다. 왜? test2 테이블의 데이터 중 일부가 참조하기 때문이다. */
