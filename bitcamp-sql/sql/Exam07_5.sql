...
/* 즉 inner join은 기준 커럼의 값이 일치할 때만 데이터를 연결한다. */
...
/* 강의명과 강의실 테이블에서 지점며오가 강의실명을 가져오자! */
select lno,
       l.titl,
       r.rno,
       r.loc,
 ...
 * 강의실이 아직 지정되지 않은 강의의 경우
 * 강의실 테이블의 데이터와 연결하지 못해 결과로 출력되지 않는 문제가 있다.
 */
